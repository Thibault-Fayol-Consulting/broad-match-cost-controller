...
function main() {
    ...
    if (totalCost > 0) {
        ...
        Logger.log("Broad match consumes " + broadPct.toFixed(2) + "% of Keyword budget (" + broadCost.toFixed(2) + ")");
        if (broadPct > CONFIG.MAX_BROAD_PERCENT) {
            Logger.log("⚠️ Broad Match is spending too much! Review immediately.");
        }
    }
}
